...
std::tuple<NotNull<const Encoding*>, int32_t>
...
nsresult nsHtml5StreamParser::SniffStreamBytes(Span<const uint8_t> aFromSegment,
  ...
  static uint8_t utf16leXml[] = {0x3C, 0x00, 0x3F, 0x00, 0x78};
  static uint8_t utf16beXml[] = {0x00, 0x3C, 0x00, 0x3F, 0x00};
  ...
  if (mBomState == BOM_SNIFFING_OVER) {
    if (mMode == VIEW_SOURCE_XML && mStartsWithLtQuestion &&
      ...
      if (encoding) {
        ...
        // This can only happen if the first buffer was a lone '<'.
...
bool nsHtml5StreamParser::internalEncodingDeclaration(nsHtml5String aEncoding) {
  ...
  if (!mLookingForMetaCharset) {
    if (mInitialEncodingWasFromParentFrame) {
      mTreeBuilder->MaybeComplainAboutCharset("EncMetaTooFrame", true,
                                              mTokenizer->getLineNumber());
    } else {
  ...
  if (mTemplatePushedOrHeadPopped) {
    mTreeBuilder->MaybeComplainAboutCharset("EncMetaAfterHead", false,
                                            mTokenizer->getLineNumber());
  }
 ...
 *    This implementation consider the foreign content rules in
 *    order to be able to use the same tree builder run for meta
 *    and the observable DOM building. Note that since <svg> and
 *    <math> imply the end of head, this only matter for meta after
 ...
 * scanned for a bogo XML declaration.
 *
 * If neither a meta nor a bogo XML declaration has been honored
 * and we aren't inheriting the encoding from a same-origin parent
 * or parsing for XHR, chardetng is used. chardetng runs first for
 * the part of the document that was searched for meta and then at
 * EOF. The part searched for meta is defined as follows in order
 * to avoid network buffer boundary-dependent behavior:
 *
...
bool nsHtml5StreamParser::ProcessLookingForMetaCharset(bool aEof) {
  ...
  if ((mNumBytesBuffered == UNCONDITIONAL_META_SCAN_BOUNDARY &&
    ...
    atKilobyte = true;
    mTokenizer->PublicAtKilobyteBoundary();
  }
  if (!mNeedsEncodingSwitchTo &&
      (aEof || (mTemplatePushedOrHeadPopped &&
                !mTokenizer->PublicIsInTokenStartedAtKilobyteBoundary() &&
                (atKilobyte ||
                 mNumBytesBuffered > UNCONDITIONAL_META_SCAN_BOUNDARY)))) {
...
void nsHtml5StreamParser::ContinueAfterScripts(nsHtml5Tokenizer* aTokenizer,
  ...
  MOZ_ASSERT(NS_IsMainThread(), "Wrong thread!");
  MOZ_ASSERT(mMode != VIEW_SOURCE_XML,
             "ContinueAfterScripts called in XML view source mode!");
  MOZ_ASSERT(!(aTokenizer && mMode == VIEW_SOURCE_HTML),
             "ContinueAfterScripts called with non-null tokenizer in HTML view "
             "source mode.");
