...
void HTMLFormElement::MaybeSubmit(Element* aSubmitter) {
  ...
  nsCOMPtr<nsIContent> asContent =
      this->AsContent();  // required by static analysis
  EventDispatcher::DispatchDOMEvent(asContent, nullptr, event, nullptr,
                                    &status);
}
...
void HTMLFormElement::MaybeReset(Element* aSubmitter) {
  ...
  nsCOMPtr<nsIContent> asContent =
      this->AsContent();  // required by static analysis
  EventDispatcher::DispatchDOMEvent(asContent, &event, nullptr, nullptr,
                                    &status);
}
