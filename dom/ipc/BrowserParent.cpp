...
/* static */
BrowserParent* BrowserParent::sFocusFromPushPop = nullptr;
...
void BrowserParent::PushFocus(BrowserParent* aBrowserParent) {
  ...
  if (aBrowserParent && !aBrowserParent->GetBrowserBridgeParent()) {
    ...
    sFocusFromPushPop = aBrowserParent;
    BrowserParent* bp = UpdateFocus();
    if (old != bp) {
      LOGBROWSERFOCUS(("PushFocus updated focus; old: %p, new: %p", old, bp));
...
void BrowserParent::PopFocus(BrowserParent* aBrowserParent) {
  BrowserParent* old = GetFocused();
  if (sFocusFromPushPop == aBrowserParent) {
    // top-level Web content
    sFocusFromPushPop = nullptr;
    sFocus = nullptr;
    if (!old) {
      LOGBROWSERFOCUS(("PushFocus moved focus to chrome; old: %p", old));
...
BrowserParent* BrowserParent::UpdateFocus() {
  if (!sFocusFromPushPop) {
    sFocus = nullptr;
    return sFocus;
  }
  nsFocusManager* fm = nsFocusManager::GetFocusManager();
  if (fm) {
    BrowsingContext* bc = fm->GetFocusedBrowsingContextChromeCache();
    if (bc) {
      ...
      if (globalTop) {
        ...
        if (sFocusFromPushPop == globalTopParent) {
